...
namespace CCompiler.ABT
{
    ...
    public sealed partial class TypeCast : Expr
    {
            ...
            switch (from)
            {
                case ExprTypeKind.CHAR:
                    switch (to)
                    {
                        ...
                        case ExprTypeKind.ULONG:
                            return new TypeCast(TypeCastType.INT8_TO_int, expr, type);
                        ...
                        case ExprTypeKind.FLOAT:
                            // char -> long -> float
                            return new TypeCast(TypeCastType.int_TO_FLOAT, new TypeCast(TypeCastType.INT8_TO_int, expr, new LongType(type.IsConst, type.IsVolatile)), type);

                        case ExprTypeKind.DOUBLE:
                            // char -> long -> double
                            return new TypeCast(TypeCastType.int_TO_DOUBLE, new TypeCast(TypeCastType.INT8_TO_int, expr, new LongType(type.IsConst, type.IsVolatile)), type);
                ...
                case ExprTypeKind.SHORT:
                    switch (to)
                    {
                        ...
                        case ExprTypeKind.LONG:
                        case ExprTypeKind.ULONG:
                            return new TypeCast(TypeCastType.INT16_TO_int, expr, type);

                        case ExprTypeKind.FLOAT:
                            // short -> long -> float
                            return new TypeCast(TypeCastType.int_TO_FLOAT, new TypeCast(TypeCastType.INT16_TO_int, expr, new LongType(type.IsConst, type.IsVolatile)), type);

                        case ExprTypeKind.DOUBLE:
                            // short -> long -> double
                            return new TypeCast(TypeCastType.int_TO_DOUBLE, new TypeCast(TypeCastType.INT16_TO_int, expr, new LongType(type.IsConst, type.IsVolatile)), type);
                ...
                case ExprTypeKind.LONG:
                    switch (to)
                    {
                        ...
                        case ExprTypeKind.SHORT:
                            if (expr.IsConstExpr)
                            {
                                return new ConstShort((short)((ConstLong)expr).Value, env);
                            }
                        ...
                        case ExprTypeKind.USHORT:
                            if (expr.IsConstExpr)
                            {
                                return new ConstUShort((ushort)((ConstLong)expr).Value, env);
                            }
                            return new TypeCast(TypeCastType.PRESERVE_INT16, expr, type);
                        ...
                        case ExprTypeKind.ULONG:
                            if (expr.IsConstExpr)
                            {
                                return new ConstULong((uint)((ConstLong)expr).Value, env);
                            }
                            return new TypeCast(TypeCastType.NOP, expr, type);
                        ...
                        case ExprTypeKind.FLOAT:
                            ...
                            return new TypeCast(TypeCastType.int_TO_FLOAT, expr, type);
                        ...
                        case ExprTypeKind.DOUBLE:
                            ...
                            return new TypeCast(TypeCastType.int_TO_DOUBLE, expr, type);
            ...
            switch (from)
            {
                case ExprTypeKind.UCHAR:
                    switch (to)
                    {
                        ...
                        case ExprTypeKind.LONG:
                        case ExprTypeKind.ULONG:
                            return new TypeCast(TypeCastType.UINT8_TO_Uint, expr, type);

                        case ExprTypeKind.FLOAT:
                            // uchar -> ulong -> long -> float
                            return new TypeCast(TypeCastType.int_TO_FLOAT, new TypeCast(TypeCastType.UINT8_TO_Uint, expr, new LongType(type.IsConst, type.IsVolatile)), type);

                        case ExprTypeKind.DOUBLE:
                            // uchar -> ulong -> long -> double
                            return new TypeCast(TypeCastType.int_TO_DOUBLE, new TypeCast(TypeCastType.UINT8_TO_Uint, expr, new LongType(type.IsConst, type.IsVolatile)), type);
                ...
                case ExprTypeKind.USHORT:
                    switch (to)
                    {
                        ...
                        case ExprTypeKind.LONG:
                        case ExprTypeKind.ULONG:
                            return new TypeCast(TypeCastType.UINT16_TO_Uint, expr, type);

                        case ExprTypeKind.FLOAT:
                            // ushort -> ulong -> long -> float
                            return new TypeCast(TypeCastType.int_TO_FLOAT, new TypeCast(TypeCastType.UINT16_TO_Uint, expr, new LongType(type.IsConst, type.IsVolatile)), type);

                        case ExprTypeKind.DOUBLE:
                            // ushort -> ulong -> long -> double
                            return new TypeCast(TypeCastType.int_TO_DOUBLE, new TypeCast(TypeCastType.UINT16_TO_Uint, expr, new LongType(type.IsConst, type.IsVolatile)), type);
                ...
                case ExprTypeKind.ULONG:
                    switch (to)
                    {
                        ...
                        case ExprTypeKind.SHORT:
                            if (expr.IsConstExpr)
                            {
                                return new ConstLong((short)((ConstULong)expr).Value, env);
                            }
                            return new TypeCast(TypeCastType.PRESERVE_INT16, expr, type);
                        ...
                        case ExprTypeKind.USHORT:
                            if (expr.IsConstExpr)
                            {
                                return new ConstULong((ushort)((ConstULong)expr).Value, env);
                            }
                            return new TypeCast(TypeCastType.PRESERVE_INT16, expr, type);
                        ...
                        case ExprTypeKind.LONG:
                            if (expr.IsConstExpr)
                            {
                                return new ConstLong((int)((ConstULong)expr).Value, env);
                            }
                            return new TypeCast(TypeCastType.NOP, expr, type);
                ...
                case ExprTypeKind.FLOAT:
                    switch (to)
                    {
                        case ExprTypeKind.CHAR:
                            ...
                            return new TypeCast(TypeCastType.PRESERVE_INT8, new TypeCast(TypeCastType.FLOAT_TO_int, expr, new LongType(type.IsConst, type.IsVolatile)), type);

                        case ExprTypeKind.SHORT:
                            if (expr.IsConstExpr)
                            {
                                return new ConstLong((short)((ConstFloat)expr).Value, env);
                            }
                            return new TypeCast(TypeCastType.PRESERVE_INT16, new TypeCast(TypeCastType.FLOAT_TO_int, expr, new LongType(type.IsConst, type.IsVolatile)), type);

                        case ExprTypeKind.USHORT:
                            if (expr.IsConstExpr)
                            {
                                return new ConstULong((ushort)((ConstFloat)expr).Value, env);
                            }
                            return new TypeCast(TypeCastType.PRESERVE_INT16, new TypeCast(TypeCastType.FLOAT_TO_int, expr, new LongType(type.IsConst, type.IsVolatile)), type);
                        ...
                        case ExprTypeKind.LONG:
                            if (expr.IsConstExpr)
                            {
                                return new ConstLong((int)((ConstFloat)expr).Value, env);
                            }
                            return new TypeCast(TypeCastType.FLOAT_TO_int, expr, type);

                        case ExprTypeKind.ULONG:
                            if (expr.IsConstExpr)
                            {
                                return new ConstULong((uint)((ConstFloat)expr).Value, env);
                            }
                            return new TypeCast(TypeCastType.FLOAT_TO_int, expr, type);
                ...
                case ExprTypeKind.DOUBLE:
                    switch (to)
                    {
                        ...
                        case ExprTypeKind.SHORT:
                            ...
                            if (expr.IsConstExpr)
                            {
                                return new ConstLong((short)((ConstDouble)expr).Value, env);
                            }
                        ...
                        case ExprTypeKind.LONG:
                            ...
                            if (expr.IsConstExpr)
                            {
                                return new ConstLong((int)((ConstDouble)expr).Value, env);
                            }
                            return new TypeCast(TypeCastType.DOUBLE_TO_int, expr, type);

                        case ExprTypeKind.ULONG:
                            if (expr.IsConstExpr)
                            {
                                return new ConstULong((uint)((ConstDouble)expr).Value, env);
                            }
                            return new TypeCast(TypeCastType.DOUBLE_TO_int, expr, type);
                        ...
                        case ExprTypeKind.USHORT:
                            ...
                            if (expr.IsConstExpr)
                            {
                                return new ConstULong((ushort)((ConstDouble)expr).Value, env);
                            }
                            return new TypeCast(TypeCastType.PRESERVE_INT16, new TypeCast(TypeCastType.DOUBLE_TO_int, expr, new LongType(type.IsConst, type.IsVolatile)), type);

                        case ExprTypeKind.FLOAT:
                            if (expr.IsConstExpr)
                            {
                                return new ConstFloat((float)((ConstDouble)expr).Value, env);
                            }
        ...
        public static Tuple<Expr, Expr, ExprTypeKind> UsualArithmeticConversion(Expr e1, Expr e2)
        {
            ...
            bool c1 = t1.IsConst;
            bool v1 = t1.IsVolatile;
            bool c2 = t2.IsConst;
            bool v2 = t2.IsVolatile;
