...
namespace CCompiler.AST {
    ...
    public sealed class SizeofType : Expr {
        ...
        public override ABT.Expr GetExpr(ABT.Env env) {
            ...
            return new ABT.ConstULong((uint)type.SizeOf, env);
        }
    }
    ...
    public sealed class SizeofExpr : UnaryExprOperator {
        ...
        public override ABT.Expr GetExpr(ABT.Env env) {
            ABT.Expr expr = this.Expr.GetExpr(env);
            return new ABT.ConstULong((uint)expr.Type.SizeOf, env);
        }
    }
            ...
            if (expr.IsConstExpr) {
                switch (expr.Type.Kind) {
                    ...
                    case ABT.ExprTypeKind.ULONG:
                        return new ABT.ConstLong(-(int)((ABT.ConstULong)expr).Value, env);
